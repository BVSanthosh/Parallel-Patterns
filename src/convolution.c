...
task_t read_image_and_mask(char *image_name_p) {
  ...
  printf("return: img %hu:\n", task.image); 
  printf("return: mask %hu:\n", task.mask); 
  return task;
}

unsigned short *process_image(task_t task) {
  printf("Stage3 func called\n");
  printf("param: img %hu:\n", task.image); 
  printf("param: mask %hu:\n", task.mask); 
...
int main(int argc, char **argv) {
  ...
  dim = stringToInt(argv[1]); 
  nr_images = stringToInt(argv[2]); 
  ...
  if (choice == 0) {   //runs without the parallel pattern
  ...
  } else if (choice == 1) {   //runs with the pipeline pattern
    TaskQueue* input_queue = init_queue();   //input queue for the pipeline

    for (int i = 0; i < nr_images; i++) {
      int* value = malloc(sizeof(int));
      *value = i;
      put_task(input_queue, value);    //adds task data to the input queue
    }

    beginning = get_current_time();

    TaskQueue* output_queue = create_pipeline(get_image_name, read_image_and_mask, process_image, input_queue);   //runs the pipeline pattern
